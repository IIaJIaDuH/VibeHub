...
import { ContentField, ExtraTextField, TitleField, patchDraft } from "../fields/ComposerFields";
...
export function ResourceForm({ draft, onChange }: TypeFormProps) {
  ...
  return (
    <>
      <TitleField
        value={draft.title}
        onChange={(title) => onChange(patchDraft(draft, { title }))}
      />
      <ContentField
        value={draft.content}
      ...
      {fields.map((field) => (
        <ExtraTextField
          ...
          onChange={(value) => onChange(patchDraft(draft, { extras: { [field.key]: value } }))}
        />
      ))}
    </>
  );
